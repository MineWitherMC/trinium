minetest.register_craftitem("trinium:item_bonemeal_wand", {
	...
	on_place = function(item, player, pointed_thing)
        ...
        if minetest.get_node_group(node.name, "herb") == 2 then
			minetest.set_node(crd, {name = "trinium:herb_flower_"..(minetest.get_node(crd).name:split"_"[3]), 
					param2 = node.param2})
        end
        if minetest.get_node_group(node.name, "herb") == 3 then
			minetest.set_node(crd, {name = "trinium:herb_shoot_"..(minetest.get_node(crd).name:split"_"[3]), 
					param2 = node.param2})
        end
	end,
})