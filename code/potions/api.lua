...
function pot.new_ingridient (name, def)
	...
	local function activator(item, player, pointed_thing)
		local i = 1
		local mas = {}
		local k
		while def.effects[i] do
		...
		cmsg.push_message_player(player, ("Applied effect %s %s for %s seconds")
			:format(def.effects[k][1], trinium.roman_number(def.effects[k][2]), def.effects[k][3]))
	end
...
function pot.new_herb (name, def)
	minetest.register_node("trinium:herb_sprout_"..name, {
		...
		groups = {harvested_by_hand = 3, non_silkable = 1, herb = 3, hidden_from_irp = 1},
	...
	minetest.register_node("trinium:herb_shoot_"..name, {
		...
		groups = {harvested_by_hand = 3, non_silkable = 1, herb = 2, hidden_from_irp = 1},
	...
	minetest.register_node("trinium:herb_flower_"..name, {
		...
		groups = {harvested_by_hand = 3, non_silkable = 1, herb = 1, hidden_from_irp = 1},
	...
	minetest.register_craftitem("trinium:item_seed_"..name, {
		...
		on_place = function(item, player, pointed_thing)
			...
			if table.exists(def.grows_on, function(v) return v == node.name end) then
